...
> [!NOTE]
> Help us improve! This page is still in draft form. If something looks off or you have feedback, please get in touch via
> our [Contact Us](/contact-us/) page.

## Overview

This document provides guidance for configuring and safely managing Dependabot across Ministry of Justice repositories.
...
## Quickstart checklist for Adopting Dependabot

- [ ] Ensure a basic `.github/dependabot.yml` exists your repository (inherited from your template or new file).
- [ ] Have a quick perusal over the [official docs](https://docs.github.com/en/code-security/dependabot)
- [ ] Configure entries for all relevant package ecosystems
- [ ] Enable Dependabot security updates in repository settings
- [ ] Review below guidance to avoid any common pitfalls
- [ ] Set appropriate cooldown values (use the recommended defaults mentioned below)
...
- Focused on keeping dependencies current, not just secure
- Lower urgency than security updates

> [!TIP]
> It's worth enabling both security updates _and_ version updates. Security updates catch known vulnerabilities, version updates help prevent
> dependencies from drifting so far that upgrades become painful.
...
`daily` is the recommended default for MoJ repositories. Longer intervals reduce the frequency at which version update PRs are generated,
which compounds dependency drift and delays awareness of newly available patches.

> [!NOTE]
> Schedule only affects version updates. Security alerts always bypass the schedule and are raised immediately regardless of this setting.
> That said, a less frequent schedule does increase the lag between a vulnerability being patched upstream and Dependabot raising a version update PR,
> particularly relevant for dependencies not yet covered by the GitHub Advisory Database.

| Interval  | Guidance                                                                                                                                                    |
|-----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `daily`   | **Recommended default.** Use for all repositories.                                                                                                          |
...
For application dependencies, a non-`daily` schedule is really only appropriate when the repository is not yet a live product and exists solely in a
development environment with no external exposure. Even then, `weekly` is a reasonable upper limit and `monthly` is best avoided.
Once a service moves toward production, switching back to `daily` is highly recommended.

> [!TIP]
> If PR volume feels unmanageable, [cooldown settings](#cooldown-strategy) and [grouping](#grouping-related-updates) are the right tools.
> They reduce noise without reducing visibility. Changing the schedule interval means potentially missing updates for longer.
...
| `semver-patch-days`    | 3    | Patches are low-risk but a brief delay helps avoid churn from rapid successive releases.                    |
...
> [!WARNING]
> When the limit is reached, Dependabot stops opening new version update PRs entirely until existing ones are merged or closed.
...
then use other tools rather than reducing the limit. Lowering the limit silently queues updates rather than making them visible.

> [!TIP]
> Keeping ecosystem entries at **5 or above** is advisable. If the current volume feels unmanageable, improving triage cadence,
> [grouping](#grouping-related-updates), or adjusting [cooldown settings](#cooldown-strategy) tend to be more sustainable solutions than lowering the limit.
...
> [!WARNING]
> `directory` is exclusive, not a starting point. Setting `directory: "/xyz"` means Dependabot **only** scans that path.
> Any dependency manifests outside it will receive neither version updates nor security update PRs.
...
The practical effect is that Dependabot appears enabled, but large portions of the repository may be unmonitored.
It's worth double-checking that all dependency locations are covered.
...
Limiting `directory` scope can make sense for gradual rollout on large legacy repositories, experimental sub-projects, or vendored third-party code.
In those cases, documenting the omission with a review date and treating it similarly to an `ignore` rule helps keep the gap visible and intentional.

> [!IMPORTANT]
> If the number of configured directories is fewer than the number of directories containing dependency manifests, it's worth treating that as a gap
> to address and documenting the reason.
...
> [!CAUTION]
> Suppressing Dependabot PRs reduces security visibility. `ignore`, `allow` and similar rules work best when they're rare, deliberate and temporary.
> [Grouping](#grouping-related-updates) and cooldown settings are usually a better first step for managing PR volume.
...
**Updates that are best not ignored:**

> [!CAUTION]
>
> - Any update raised as a **security alert** bypass cooldown by design and silencing them removes an important signal
> - Updates assumed to be low-risk but exploitability assessments can change over time
> - For major version updates, the cooldown and triage process is a more reliable way to manage the effort involved

#### How to Use `ignore` Safely

When `ignore` is genuinely needed, including a comment with the justification and a target removal date makes future audits much easier.
Rules without context tend to outlive their usefulness.
...
> [!WARNING]
> Using `allow` inverts Dependabot's default behaviour into an opt-in model.
> Any dependency not matched by an `allow` rule will receive **no** security update PRs.
> It's worth having a documented reason before reaching for it.
...
In both cases, including a comment with a justification and a target removal or review date makes future audits much easier:
...
It's a good idea to review `allow` rules at the same time as `ignore` rules as both represent deliberate gaps in update coverage.
...
- **Production dependencies are best reviewed individually**: grouping them increases the chance of a security fix getting lost in a batch PR.
- **Major version updates are worth keeping separate**: they often need individual changelog review and may require code changes.
- **Dependencies with open security alerts are best kept out of groups**: their fix PR is easier to track, prioritise and revert when it stands alone.
...
> [!WARNING] `lockfile-only` carries significant security risk and is best avoided in MoJ repositories.
...
The only scenario where `lockfile-only` may be acceptable is as a **short-term, explicitly tracked tactical decision** during a complex dependency
resolution problem, with a tracked issue to remove it. Treating it as a permanent configuration is best avoided.
...
> \[!TIP] Prefer OIDC authentication for any registry that supports it. Static credentials (tokens, passwords) must be stored as
...
Retaining the `dependencies` and ecosystem labels is important. Creating `major`, `minor` and `patch` labels is also recommended,
they enable filtering and automation that makes triaging much easier. Additional labels can be configured per ecosystem:
...
Dependabot PRs have no assignees by default. Individual assignees can be added in `dependabot.yml`, but teams cannot be assigned this way.
At scale, relying on individual assignees tends to create gaps. It increases PR abandonment risk and doesn't adapt well to team changes.
Assignees work well for documented security on-call rotations, for everything else, CODEOWNERS is a more robust option.

### Reviewers: Use CODEOWNERS

> [!IMPORTANT]
> GitHub removed the `reviewers:` option from `dependabot.yml` in 2025. CODEOWNERS is now the way to assign reviewers to Dependabot PRs.

Having a valid `CODEOWNERS` file with team-based ownership (rather than individual usernames where possible) helps ensure Dependabot PRs:
...
Without CODEOWNERS, Dependabot PRs have no required reviewers and can be merged without appropriate oversight, this has both compliance and
security implications worth bearing in mind.
